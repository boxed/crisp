...
public:
  ...
  BOOL Init(LPCSTR szSymPath)
  {
    ...
    if (m_hDbhHelp == NULL)
	{
	  Trace(_FMT(_T("Failed to load dbghelp libary: %s"), GetLastErrorString().c_str()));
      return FALSE;
	}
...
private:
  ...
  BOOL GetModuleListPSAPI(HANDLE hProcess, DWORD pid)
  {
    ...
    if (hPsapi == NULL)
	{
	  Trace(_FMT(_T("Failed to load psapi.dll: %s"), GetLastErrorString().c_str()));
      return FALSE;
	}
    ...
    if ( (pEPM == NULL) || (pGMFNE == NULL) || (pGMBN == NULL) || (pGMI == NULL) )
    {
	  ...
	  Trace(_T("Not all functions in psapi.dll found"));
      FreeLibrary(hPsapi);
      return FALSE;
    }
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  ...
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    if (entry.lineFileName[0] == 0)
    {
      ...
      _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%p (%s): %s", (LPVOID) entry.offset, entry.moduleName, entry.name);
    }
    else
