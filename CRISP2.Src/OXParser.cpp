...
static String TrimLeft(LPCTSTR lpszTargets)
{
	// find first non-space character
//	CopyBeforeWrite();
	String str("");
	LPCTSTR lpsz = lpszTargets;
...
//		int nDataLength = _tcslen(lpszTargets)/*GetData()->nDataLength*/ - (lpsz - lpszTargets);
//		memmove(lpszTargets, lpsz, (nDataLength + 1)*sizeof(TCHAR));
					...
					if (ch==TCHAR(0x3e))
					{
						if (sEntity.size()>8)
						{
							int nRet=CompareString(LOCALE_SYSTEM_DEFAULT,
								NORM_IGNORECASE,(LPCTSTR) sEntity.c_str(),8,
								_T("!ENTITY "),8);
							
							if (CSTR_EQUAL==nRet)
							{
								//found entity
//								sEntity = String(sEntity, 8, .Right(sEntity.size()-8);
								sEntity = String(sEntity, 8, sEntity.size()-8);
								String sLiteral = sEntity;
...
//									sEntity=sEntity.Left(nSpace);
									sEntity = String(sEntity.c_str(), nSpace);
//									sLiteral=sLiteral.Right(sLiteral.size()-nSpace);
									sLiteral = String(sLiteral, nSpace, sLiteral.size()-nSpace);
//									sLiteral.TrimLeft();
									sLiteral = TrimLeft(sLiteral.c_str());
									m_EntityTable.Add((LPCTSTR) sEntity.c_str(),(DWORD) pData);
								}
							}
						}
						sEntity="";
...
BOOL COXParser::WriteFile(LPCTSTR szFile)
{
	...
	{
		Trace(_FMT(_T("Failed to open file '%s' for writing: %s"), szFile, GetLastErrorString().c_str()));
		return FALSE;
	}
...
LPTSTR COXParser::LoadFile(LPCTSTR szFile)
{
	...
	if (INVALID_HANDLE_VALUE == hFile)
	{
		Trace(_FMT(_T("Failed to open file '%s' for reading: %s"), szFile, GetLastErrorString().c_str()));
		return NULL;
	}
